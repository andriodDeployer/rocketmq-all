...
public class PullMessageProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        if (hasSubscriptionFlag) {//有设置subscribe flag,表示第一次pull或者需要更新filter
            try {
        ...
        } else {//3、没有设置subscribe flag，表示之前已经订阅过了,对比订阅条件是否一致
...
//每次都会判断的。
            ...
            switch (this.brokerController.getMessageStoreConfig().getBrokerRole()) {
                ...
                case SYNC_MASTER:
                    break;
                case SLAVE:
                    if (!this.brokerController.getBrokerConfig().isSlaveReadEnable()) {//如果当前就是slave的话，且设置的salve不可读。
                        response.setCode(ResponseCode.PULL_RETRY_IMMEDIATELY);
                        responseHeader.setSuggestWhichBrokerId(MixAll.MASTER_ID);//从
                    }
                    break;
            }
