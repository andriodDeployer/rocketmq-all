...
//对于以上的数据结构，主要看的是维护这些数据结构的代码，也就是：who，when，what写进去。who when读数据。
    ...
    public TopicRouteData pickupTopicRouteData(final String topic) {
        ...
        try {
            try {
                this.lock.readLock().lockInterruptibly();//添加读写锁，保证读写一致性
                List<QueueData> queueDataList = this.topicQueueTable.get(topic);//找到topic中的所有的mq。mq可能分布在多个主从Broker集群中。
                if (queueDataList != null) {
                    ...
                    while (it.hasNext()) {
                        QueueData qd = it.next();
                        brokerNameSet.add(qd.getBrokerName());//找出topic的mq所在所有brokerName(也就是集群名称)
                    }

                    for (String brokerName : brokerNameSet) {
                        BrokerData brokerData = this.brokerAddrTable.get(brokerName);//根据brokername找出指定的 主从集群信息。
                        if (null != brokerData) {
